...
from .dependencies import dependency_ids_for_payload, dependent_objects, dependency_closure
from .image_recognition_import import recognize_image_bytes, reviewed_graph_from_post
from .geometry_command_registry import validate_geometry_command
from .object_type_registry import (
    get_object_type, registered_object_types, validate_registered_object_type,
    resolve_registered_position, render_registered_tikz,
)
...
def _serialize_drawing_object(obj):
    ...
    return {
        ...
        'style': obj.style,
        'order': obj.order,
        'dependencies': dependency_ids_for_payload(obj.type, obj.data),
...
def _validate_object_references(drawing, object_type, data):
    ...
    def object_type_for(object_id):
        if not isinstance(object_id, str) or not object_id.strip():
            return None
        actual_type = DrawingObject.objects.filter(
            drawing=drawing, object_id=object_id
        ).values_list('type', flat=True).first()
        if actual_type == 'geometry.midpoint':
            return 'geometry.point'
        definition = get_object_type(actual_type)
        if definition and definition.point_like:
            return 'geometry.point'
        return actual_type
    ...
    if object_type == 'geometry.midpoint':
        errors.update(validate_geometry_command(
            command_id='midpoint', object_type=object_type, data=data, object_type_for=object_type_for
        ))

    plugin_errors = validate_registered_object_type(
        drawing=drawing, object_type=object_type, data=data, object_type_for=object_type_for
    )
    ...
    if object_type == 'label.relative':
        base_id = data.get('baseObjectId')
        actual_type = object_type_for(base_id)
        actual_definition = get_object_type(actual_type) if actual_type else None
        actual_is_point_like = actual_type in {'graph.vertex', 'geometry.point', 'geometry.midpoint'} or bool(actual_definition and actual_definition.point_like)
        if not actual_is_point_like:
            errors['baseObjectId'] = 'baseObjectId must reference an existing positioned point-like object.'
        if not isinstance(data.get('text'), str) or not data.get('text', '').strip():
            errors['text'] = 'label.relative requires non-empty text.'
        for field in ('dx', 'dy'):
            try:
    ...
    if object_type == 'group':
        child_ids = data.get('childObjectIds')
        if not isinstance(child_ids, list) or not child_ids:
            errors['childObjectIds'] = 'group requires a non-empty childObjectIds list.'
        else:
            normalized_ids = []
            for index, child_id in enumerate(child_ids):
                if not isinstance(child_id, str) or not child_id.strip():
                    errors[f'childObjectIds[{index}]'] = 'Each group child must be a non-empty object identifier.'
                    continue
                normalized_ids.append(child_id)
                if object_type_for(child_id) is None:
                    errors[f'childObjectIds[{index}]'] = 'Each group child must reference an existing object.'
            if len(normalized_ids) != len(set(normalized_ids)):
                errors['childObjectIds'] = 'A group cannot contain the same object more than once.'
    ...
    dependency_ids = dependency_ids_for_payload(object_type, data)
    if any(not isinstance(item, str) or not item.strip() for item in dependency_ids):
        errors['dependencies'] = 'Object dependencies must contain non-empty object identifiers.'
...
DRAWING_MODE_ALLOWED_TYPES = {
    Drawing.MODE_GRAPH: {'graph.vertex', 'graph.edge', 'text.latex', 'label.relative', 'group'},
    Drawing.MODE_GEOMETRY: {'geometry.point', 'geometry.midpoint', 'geometry.segment', 'geometry.circle', 'geometry.polygon', 'text.latex', 'label.relative', 'group'},
    Drawing.MODE_PLOT: {'plot.series', 'plot.chart', 'group'},
    Drawing.MODE_MIXED: {'graph.vertex', 'graph.edge', 'geometry.point', 'geometry.midpoint', 'geometry.segment', 'geometry.circle', 'geometry.polygon', 'text.latex', 'label.relative', 'plot.series', 'plot.chart', 'group'},
}

for definition in registered_object_types():
    for mode in definition.modes:
        DRAWING_MODE_ALLOWED_TYPES.setdefault(mode, set()).add(definition.type_id)
...
def _tikz_point_reference_map(objects):
    ...
    return {
        ...
        or bool(get_object_type(obj.type) and get_object_type(obj.type).point_like)
    }
...
def build_drawing_tikz(drawing):
    ...
    - graph.vertex,
    - geometry.midpoint,
    - typy punktowe zarejestrowane przez pluginy,
    - geometry.ratio_point,
    - geometry.segment,
    - graph.edge,
    - geometry.circle,
    - geometry.polygon,
    ...
    def resolve_point_position(obj, visited=None):
        if not obj:
            return None
        ...
        if obj.type == 'geometry.midpoint':
            source_position = resolve_point_position(objects_by_id.get(data.get('source')), visited)
            target_position = resolve_point_position(objects_by_id.get(data.get('target')), visited)
            if not source_position or not target_position:
                return None
            return ((source_position[0] + target_position[0]) / 2, (source_position[1] + target_position[1]) / 2)
        return resolve_registered_position(
            obj,
            objects_by_id=objects_by_id,
            resolve_position=lambda dependency: resolve_point_position(dependency, visited),
        )

    for obj in objects:
        if obj.type not in {'geometry.point', 'geometry.midpoint', 'graph.vertex'} and not bool(get_object_type(obj.type) and get_object_type(obj.type).point_like):
            continue

        data = obj.data or {}
        style = obj.style or {}
        try:
            position = resolve_point_position(obj)
            if not position:
                raise ValueError('missing derived-point input')
            raw_x, raw_y = position
            x, y = _tikz_coord_from_svg(raw_x, raw_y, canvas_height=canvas_height, scale=tikz_scale)
        except (TypeError, ValueError):
            body_lines.append(f"  % Skipped {obj.object_id}: invalid point coordinates")
            continue

        custom_lines = render_registered_tikz(
            obj,
            position=(x, y),
            context={
                "safe_identifier": _tikz_safe_identifier,
                "format_number": _format_tikz_number,
                "color": tikz_color,
                "style_number": tikz_style_number,
                "opacity": tikz_opacity,
            },
        )
        if custom_lines is not None:
            body_lines.extend(custom_lines)
            continue
    ...
    for obj in objects:
        if obj.type != 'label.relative':
            continue

        data = obj.data or {}
        style = obj.style or {}
        if not object_is_visible(obj) or style.get('showLabel', True) is False:
            continue
        base = points_by_id.get(data.get('baseObjectId'))
        if not base:
            body_lines.append(f"  % Skipped {obj.object_id}: missing base object")
            continue
        base_data = base.data or {}
        try:
            base_x = float(base_data.get('x'))
            base_y = float(base_data.get('y'))
            dx = float(data.get('dx', 0))
            dy = float(data.get('dy', 0))
            x, y = _tikz_coord_from_svg(base_x + dx, base_y + dy, canvas_height=canvas_height, scale=tikz_scale)
        except (TypeError, ValueError):
            body_lines.append(f"  % Skipped {obj.object_id}: invalid relative label coordinates")
            continue
        label_text = data.get('text') or ''
        if not label_text:
            continue
        ...
        body_lines.append(f"  \\node[{options}] at ({_format_tikz_number(x)}, {_format_tikz_number(y)}) {{$ {label_text} $}};")
...
class DrawingDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['is_text_mode'] = self.object.mode in (Drawing.MODE_GRAPH, Drawing.MODE_GEOMETRY, Drawing.MODE_MIXED)
...
def import_drawing_image(request):
    """Recognize a graph image, show a review screen, then import approved objects."""
    context = {'errors': None, 'preview': None}
    if request.method == 'GET':
        return render(request, 'routes/drawing_image_import.html', context)

    action = request.POST.get('action', 'preview')
    if action == 'preview':
        uploaded = request.FILES.get('image_file')
        if not uploaded:
            context['errors'] = {'image_file': 'Wybierz plik PNG albo JPG.'}
            return render(request, 'routes/drawing_image_import.html', context, status=400)
        try:
            preview = recognize_image_bytes(uploaded.read(), uploaded.name)
        except ValueError as exc:
            context['errors'] = {'image_file': str(exc)}
            return render(request, 'routes/drawing_image_import.html', context, status=400)
        preview['graph_json'] = json.dumps(preview['graph'], ensure_ascii=False)
        vertices_by_id = {str(item.get('id')): item for item in preview['graph'].get('vertices', [])}
        preview['display_edges'] = [
            {**edge, 'index': index, 'source_vertex': vertices_by_id.get(str(edge.get('source'))), 'target_vertex': vertices_by_id.get(str(edge.get('target')))}
            for index, edge in enumerate(preview['graph'].get('edges', []))
            if vertices_by_id.get(str(edge.get('source'))) and vertices_by_id.get(str(edge.get('target')))
        ]
        context['preview'] = preview
        context['title'] = request.POST.get('title', '').strip() or f"Graf z obrazu — {preview['filename']}"
        return render(request, 'routes/drawing_image_import.html', context)

    if action != 'import':
        context['errors'] = {'action': 'Nieznana operacja.'}
        return render(request, 'routes/drawing_image_import.html', context, status=400)

    try:
        graph = json.loads(request.POST.get('graph_json', ''))
        reviewed_graph = reviewed_graph_from_post(graph, request.POST)
        width = int(request.POST.get('canvas_width', 1000))
        height = int(request.POST.get('canvas_height', 600))
        from experiments.image_recognition.to_route_editor import graph_to_route_editor_document
        document = graph_to_route_editor_document(
            reviewed_graph,
            title=request.POST.get('title', '').strip() or 'Graf rozpoznany z obrazu',
            source_image=request.POST.get('source_filename', ''),
            canvas_width=max(200, min(width, 5000)),
            canvas_height=max(200, min(height, 5000)),
        )
        document.setdefault('metadata', {})['reviewed_by_user'] = True
        drawing = import_drawing_from_document(user=request.user, document=document)
    except (ValueError, json.JSONDecodeError, ValidationError) as exc:
        message = str(exc)
        if isinstance(exc, ValidationError):
            message = '; '.join(exc.messages)
        context['errors'] = {'import': message}
        return render(request, 'routes/drawing_image_import.html', context, status=400)
...
def _creation_reference_ids(value, result=None):
    """Collects $created:<clientId> references recursively from JSON data."""
    result = result if result is not None else set()
    if isinstance(value, str) and value.startswith('$created:'):
        client_id = value[len('$created:'):].strip()
        if not client_id:
            raise ValueError('Odwołanie $created: musi zawierać clientId.')
        result.add(client_id)
    elif isinstance(value, list):
        for item in value:
            _creation_reference_ids(item, result)
...
def _resolve_creation_references(value, created_by_client_id):
    if isinstance(value, str) and value.startswith('$created:'):
        client_id = value[len('$created:'):].strip()
        if client_id not in created_by_client_id:
            raise ValueError(f'Nieznany clientId w odwołaniu $created: {client_id}.')
        return created_by_client_id[client_id].object_id
    if isinstance(value, list):
        return [_resolve_creation_references(item, created_by_client_id) for item in value]
    if isinstance(value, dict):
        return {key: _resolve_creation_references(item, created_by_client_id) for key, item in value.items()}
    return value


def _order_creation_payloads(payloads):
    nodes = []
    by_client_id = {}
    for index, raw_payload in enumerate(payloads):
        if not isinstance(raw_payload, dict):
            raise ValueError('Każdy element pakietu musi być obiektem JSON.')
        raw_client_id = raw_payload.get('clientId')
        client_id = None if raw_client_id is None else str(raw_client_id).strip()
        if raw_client_id is not None and not client_id:
            raise ValueError('clientId nowego obiektu nie może być pusty.')
        if client_id:
            if client_id in by_client_id:
                raise ValueError(f'Powtórzony clientId w operacji tworzenia: {client_id}.')
        node = {
            'payload': raw_payload,
            'client_id': client_id,
            'index': index,
            'dependencies': _creation_reference_ids(raw_payload),
        }
        nodes.append(node)
        if client_id:
    ...
    for node in nodes:
        for dependency_id in node['dependencies']:
            if dependency_id not in by_client_id:
                raise ValueError(f'Nieznany clientId w odwołaniu $created: {dependency_id}.')
    ...
    def visit(node):
        ...
        if key in permanent:
            return
        if key in temporary:
            raise ValueError('Wykryto cykl zależności w pakiecie tworzonym przez plugin.')
        temporary.add(key)
        for dependency_id in node['dependencies']:
            visit(by_client_id[dependency_id])
...
@login_required
@require_POST
def drawing_objects_bulk_create(request, drawing_id):
    """Atomically creates a dependency package submitted by a plugin.

    The server resolves clientId / $created references, orders payloads
    topologically and commits all objects in one database transaction.
    """
    drawing = get_object_or_404(Drawing, id=drawing_id, user=request.user)
    try:
    ...
    payloads = body.get('objects')
    if not isinstance(payloads, list) or not payloads:
        return JsonResponse({'success': False, 'errors': {'objects': 'objects must be a non-empty list.'}}, status=400)

    try:
        ordered_nodes = _order_creation_payloads(payloads)
    except ValueError as exc:
        return JsonResponse({'success': False, 'errors': {'objects': str(exc)}}, status=400)
    ...
    try:
        with transaction.atomic():
            ...
            for node in ordered_nodes:
                raw_payload = dict(node['payload'])
                raw_payload.pop('clientId', None)
                payload = _resolve_creation_references(raw_payload, created_by_client_id)

                errors = _validate_object_payload(payload)
                if errors:
                    raise ValidationError(errors)

                object_id = (payload.get('object_id') or f"obj_{uuid.uuid4().hex[:12]}").strip()
                if DrawingObject.objects.filter(drawing=drawing, object_id=object_id).exists():
                    raise ValidationError({'object_id': 'object_id must be unique inside this drawing.'})

                object_type = payload['type'].strip()
                mode_errors = _validate_object_allowed_for_drawing_mode(drawing, object_type)
                if mode_errors:
                    raise ValidationError(mode_errors)
                reference_errors = _validate_object_references(drawing, object_type, payload.get('data', {}))
                if reference_errors:
                    raise ValidationError(reference_errors)

                order = int(payload['order']) if 'order' in payload else next_order
                ...
                created.append(obj)
                if node['client_id']:
                    created_by_client_id[node['client_id']] = obj
    except ValidationError as exc:
        message_dict = getattr(exc, 'message_dict', None)
        errors = message_dict if message_dict is not None else {'objects': exc.messages}
        return JsonResponse({'success': False, 'errors': errors}, status=400)
    except ValueError as exc:
        return JsonResponse({'success': False, 'errors': {'objects': str(exc)}}, status=400)
    ...
    return JsonResponse({
        'success': True,
        'objects': [_serialize_drawing_object(obj) for obj in created],
        'client_id_map': {
            client_id: obj.object_id for client_id, obj in created_by_client_id.items()
        },
    }, status=201)
...
def drawing_object_detail(request, drawing_id, object_id):
    ...
    if request.method == 'DELETE':
        all_objects = list(drawing.drawing_objects.all())
        cascade = dependency_closure(all_objects, [object_id])
        deleted_object_ids = [candidate.object_id for candidate in cascade]
        DrawingObject.objects.filter(drawing=drawing, object_id__in=deleted_object_ids).delete()
        return JsonResponse({
            'success': True,
