    ...
    class DrawingEditor {
        ...
        async createCircumcenter(pointIds) {
            ...
            if (new Set(pointIds).size !== 3) {
                this.pendingCircumcenterPointIds = [];
                this.render();
                this.setStatus("Do wyznaczenia środka potrzebne są trzy różne punkty.", true);
                return;
            }
            const positions = pointIds.map((id) => this.resolveObjectPosition(this.findObject(id)));
            if (positions.some((point) => !point)) {
            ...
            if (Math.abs(denominator) < 1e-9) {
                this.pendingCircumcenterPointIds = [];
                this.render();
                this.setStatus("Wybrane punkty są współliniowe — okrąg opisany nie jest jednoznaczny.", true);
                return;
            }
            const label = this.labelInput ? this.labelInput.value.trim() : "O";
            const payload = {
                type: "geometry.circumcenter",
                data: {command: "circumcenter", pointA: pointIds[0], pointB: pointIds[1], pointC: pointIds[2], label},
                style: this.styleForNewObject("geometry.circumcenter"),
            };
            try {
                const result = await this.request(this.objectsUrl, {
                    method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify(payload),
                ...
                this.setToolType("select");
                this.render();
                this.setStatus("Dodano dynamiczny środek okręgu opisanego.");
            } catch (error) {
                this.setStatus("Nie udało się dodać środka okręgu opisanego: " + error.message, true);
            }
        }
        ...
        async createNinePointCenter(pointIds) {
            ...
            if (new Set(pointIds).size !== 3) {
                this.pendingNinePointCenterPointIds = [];
                this.render();
                this.setStatus("Do wyznaczenia środka okręgu dziewięciu punktów potrzebne są trzy różne punkty.", true);
                return;
            }
            const positions = pointIds.map((id) => this.resolveObjectPosition(this.findObject(id)));
            if (positions.some((point) => !point)) {
            ...
            if (area2 < 1e-9) {
                this.pendingNinePointCenterPointIds = [];
                this.render();
                this.setStatus("Wybrane punkty są współliniowe — okrąg dziewięciu punktów nie jest określony.", true);
                return;
            }
            const label = this.labelInput ? this.labelInput.value.trim() : "N";
            const payload = {
                type: "geometry.nine_point_center",
                data: {command: "nine_point_center", pointA: pointIds[0], pointB: pointIds[1], pointC: pointIds[2], label},
                style: this.styleForNewObject("geometry.nine_point_center"),
            };
            try {
                const result = await this.request(this.objectsUrl, {
                    method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify(payload),
                ...
                this.setToolType("select");
                this.render();
                this.setStatus("Dodano dynamiczny środek okręgu dziewięciu punktów.");
            } catch (error) {
                this.setStatus("Nie udało się dodać środka okręgu dziewięciu punktów: " + error.message, true);
            }
        }
        ...
        async createIncenter(pointIds) {
            ...
            if (new Set(pointIds).size !== 3) {
                this.pendingIncenterPointIds = [];
                this.render();
                this.setStatus("Do wyznaczenia środka okręgu wpisanego potrzebne są trzy różne punkty.", true);
                return;
            }
            const positions = pointIds.map((id) => this.resolveObjectPosition(this.findObject(id)));
            if (positions.some((point) => !point)) {
            ...
            const payload = {
                type: "geometry.incenter",
                data: {command: "incenter", pointA: pointIds[0], pointB: pointIds[1], pointC: pointIds[2], label},
                style: this.styleForNewObject("geometry.incenter"),
            };
            try {
                const result = await this.request(this.objectsUrl, {
                    method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify(payload),
                ...
                this.setToolType("select");
                this.render();
                this.setStatus("Dodano dynamiczny środek okręgu wpisanego.");
            } catch (error) {
                this.setStatus("Nie udało się dodać środka okręgu wpisanego: " + error.message, true);
            }
        }
        ...
        async createExcenter(pointIds) {
            ...
            if (area2 < 1e-9) {
                this.pendingExcenterPointIds = [];
                this.render();
                this.setStatus("Wybrane punkty są współliniowe — okrąg dopisany nie jest określony.", true);
                return;
            }
            ...
            const payload = {
                type: "geometry.excenter",
                data: {command: "excenter", pointA: pointIds[0], pointB: pointIds[1], pointC: pointIds[2], oppositeVertex, label},
                style: this.styleForNewObject("geometry.excenter"),
            };
            try {
                ...
                this.setToolType("select");
                this.render();
                this.setStatus("Dodano dynamiczny środek okręgu dopisanego naprzeciw wierzchołka " + oppositeVertex + ".");
            } catch (error) {
                this.setStatus("Nie udało się dodać środka okręgu dopisanego: " + error.message, true);
            }
        }
        ...
        async deleteSelectedObject() {
            ...
            // Wyznaczamy pełne domknięcie zależności jeszcze przed pierwszym DELETE.
            // Dzięki temu historia i licznik obejmują także obiekty usunięte kaskadowo
            // przez backend (np. odcinek zależny od usuwanego punktu).
            const selectedIds = selectedObjects.map((object) => object.object_id);
            const closureIds = this.dependencyClosureIds(selectedIds);
            const deletedObjects = this.objectsInDependencyOrder(
                this.flattenObjects().filter((object) => closureIds.has(object.object_id))
            ).map((object) => this.cloneObject(object));

            try {
                const alreadyDeleted = new Set();
                const deletionOrder = [...this.objectsInDependencyOrder(selectedObjects)].reverse();
                ...
                for (const object of deletionOrder) {
                    ...
                    try {
                        const result = await this.request(this.objectDetailUrl(object.object_id), {method: "DELETE"});
                        ...
                        // Obiekt mógł zostać usunięty kaskadowo przez wcześniejszy DELETE.
                        // Tylko błąd 404 jest w tej sytuacji nieszkodliwy.
                ...
                // Serwer jest źródłem prawdy. Ponowne pobranie listy usuwa ryzyko,
                // że frontend zachowa obiekt skasowany kaskadowo albo element grupy.
                const payload = await this.request(this.objectsUrl);
                this.objects = payload.objects || [];
                this.selectedObjectIds = new Set();
                this.selectedObjectId = null;
                ...
                // Nawet po błędzie odświeżamy dane, aby ekran odpowiadał stanowi bazy.
                try {
                    const payload = await this.request(this.objectsUrl);
                    this.objects = payload.objects || [];
                    this.selectedObjectIds = new Set();
