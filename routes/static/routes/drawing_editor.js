(function () {
    ...
    function latexToSvgText(value) {
        ...
        Object.entries(greek).forEach(([command, symbol]) => { text = text.split(command).join(symbol); });
        text = text.replace(/_\{([0-9+\-]+)\}|_([0-9])/g, (_, braced, single) => String(braced || single).split("").map((char) => sub[char] || char).join(""));
        text = text.replace(/\^\{([0-9+\-]+)\}|\^([0-9])/g, (_, braced, single) => String(braced || single).split("").map((char) => sup[char] || char).join(""));
        return text.replace(/\\mathrm\{([^}]*)\}/g, "$1").replace(/[{}]/g, "");
    }
    ...
    class DrawingEditor {
        ...
        async handleCanvasClick(event) {
            ...
            try {
                ...
                this.render();
                this.setStatus(selectedType === "text.latex"
                    ? "Dodano tekst LaTeX " + result.object.object_id + ". Możesz go zaznaczać i przeciągać."
                    : "Dodano obiekt " + result.object.object_id + ".");
            } catch (error) {
        ...
        handleObjectPointerDown(event, objectId) {
            ...
            // Krok 90: etykieta przypięta jest akcją tworzącą, a nie zwykłym
            // zaznaczeniem/przeciąganiem obiektu. Wcześniej brakowało tej gałęzi,
            // więc kliknięcie obiektu tylko go zaznaczało i nic nie tworzyło.
            if (this.currentToolCreatesRelativeLabel()) {
                const text = this.labelInput ? this.labelInput.value.trim() : "";
                if (!text) {
                    this.setStatus("Najpierw wpisz treść etykiety w polu „Etykieta / tekst”, a potem kliknij obiekt bazowy.", true);
                    return;
                }
                const basePosition = this.resolveObjectPosition(object);
                if (!basePosition) {
                    this.setStatus("Etykietę przypiętą można dodać do obiektu posiadającego jednoznaczną pozycję, np. punktu lub wierzchołka.", true);
                    return;
                }
                this.createRelativeLabel(object, text);
                return;
            }
        ...
        renderRelativeLabel(object) {
            ...
            text.textContent = latexToSvgText(textValue);
            text.addEventListener("pointerdown", (event) => this.handleObjectPointerDown(event, object.object_id));
            this.svg.appendChild(text);
        }
